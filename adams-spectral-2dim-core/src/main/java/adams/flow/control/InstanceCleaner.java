 ...
 * </pre>
 *
 * <pre>-cleaner &lt;knir.data.cleaner.instance.AbstractCleaner [options]&gt; (property: cleaner)
...
public class InstanceCleaner
  ...
  public String setUp() {
	...
	result = "Accepted input and actor for rejected tokens are not compatible!";
      else if (!comp.isCompatible(accepts(), ((InputConsumer) m_RejectionMessagesActor).accepts()))
  ...
  protected String doExecute() {
    ...
    msg = null;
    try {
      msg = m_Cleaner.check((Instance) m_InputToken.getPayload());
    }
    catch (Exception e) {
      result = handleException("Failed to clean instance: " + m_InputToken.getPayload(), e);
    }
