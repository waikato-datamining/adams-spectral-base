...
public class SpectralDbBackend
  ...
  /** the backends. */
  protected adams.db.SpectralDbBackend[] m_Backends;

  /** the default backend. */
  protected adams.db.SpectralDbBackend m_NonMirroredBackend;
  ...
  public void defineOptions() {
    ...
    m_OptionManager.add(
      "backend", "backends",
      new adams.db.SpectralDbBackend[0]);

    m_OptionManager.add(
      "non-mirrored-backend", "nonMirroredBackend",
      new adams.db.mysql.SpectralDbBackend());
  }
  ...
  public void setMirrors(AbstractDatabaseConnection[] value) {
    ...
    m_Backends = (adams.db.SpectralDbBackend[]) Utils.adjustArray(m_Backends, m_Mirrors.length, new adams.db.mysql.SpectralDbBackend());
    reset();
  }
   ...
   */
  public void setBackends(adams.db.SpectralDbBackend[] value) {
    m_Backends = value;
   ...
   * @return		the backends
   */
  public adams.db.SpectralDbBackend[] getBackends() {
    return m_Backends;
  }
   ...
   */
  public void setNonMirroredBackend(adams.db.SpectralDbBackend value) {
   ...
   * @return		the backend
   */
  public adams.db.SpectralDbBackend getNonMirroredBackend() {
   ...
   */
  protected adams.db.SpectralDbBackend getBackendFor(AbstractDatabaseConnection conn) {
    int		i;
  ...
  public SpectrumIntf getSpectrum(AbstractDatabaseConnection conn) {
    if (canMirror(conn))
      return SpectrumT.getSingleton(conn, getBackendFor(conn).getSpectrum(getMirrorFor(conn)));
    else
      return m_NonMirroredBackend.getSpectrum(conn);
  }
  ...
  public SampleDataIntf getSampleData(AbstractDatabaseConnection conn) {
    if (canMirror(conn))
      return SampleDataT.getSingleton(conn, getBackendFor(conn).getSampleData(getMirrorFor(conn)));
    else
      return m_NonMirroredBackend.getSampleData(conn);
  }
}
