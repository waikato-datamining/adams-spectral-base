...
/*
 * CosineSimilarityBasedThreshold.java
 ...
 * between the spectra. Spectra that go beyond the specified threshold
 * with other spectra get removed.
 *
 * @author fracpete (fracpete at waikato dot ac dot nz)
 */
public class CosineSimilarityBasedThreshold
  extends AbstractOutlierRemoval {

  private static final long serialVersionUID = -7893869545728509403L;
  ...
  public String globalInfo() {
    return "Applies the pre-filter and then computes the cosine similarity "
  ...
  public String thresholdTipText() {
    return "The threshold to use for the cosine similarity.";
  }
  ...
  public String belowCountThresholdTipText() {
    return "For each spectrum the number of times it falls below the CS threshold is counted and this threshold "
  ...
  protected MultiSpectrum doRemoveOutliers(MultiSpectrum multi, MessageCollection errors) {
	...
	if (n == i)
	  continue;
	cs = StatUtils.cosineSimilarity(ampl[i], ampl[n]);
	if (cs < m_Threshold)
	  count[i]++;
	if (isLoggingEnabled())
	  getLogger().info(i + " vs " + n + ": cs/" + cs + " < threshold/" + m_Threshold + " = " + (cs < m_Threshold));
      }
    }
    if (isLoggingEnabled())
    ...
    for (i = 0; i < count.length; i++) {
      // CS not below threshold for all other spectra? -> add
