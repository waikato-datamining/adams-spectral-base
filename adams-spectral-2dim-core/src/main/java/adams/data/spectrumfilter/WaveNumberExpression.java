...
/**
 <!-- globalinfo-start -->
 * Applies the specified mathematical expression to each wave number.<br>
...
public class WaveNumberExpression
  ...
  public void defineOptions() {
    ...
    m_OptionManager.add(
      ...
      new MathematicalExpressionText(PLACEHOLDER_WAVENUMBER));
  }
  ...
  protected Spectrum processData(Spectrum data) {
	...
	parser.parse();
	newWave = parser.getResult();
	if (newWave != null)
	  result.add(new SpectrumPoint(newWave.floatValue(), points.get(i).getAmplitude()));
      }
    }
    catch (Exception e) {
