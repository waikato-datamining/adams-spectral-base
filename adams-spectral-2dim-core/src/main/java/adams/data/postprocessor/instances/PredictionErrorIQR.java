...
public class PredictionErrorIQR
  ...
  protected Instances cleanData(Instances data, Classifier classifier, int numFolds, long seed, double iqrMultiplier) {
	...
	if (m_UseAbsoluteError)
      ...
      iqr   = p75 - p25;
      if (isLoggingEnabled())
	getLogger().info("p25=" + p25 + ", p75=" + p75 + ", iqr=" + iqr);
      upper = p75 + iqr * iqrMultiplier;
      if (isLoggingEnabled())
	getLogger().info("upper=" + upper);
  ...
  protected Instances iterate(Instances data, Classifier classifier, int numFolds, long seed, double iqrMultiplier, int numIterations, int maxNonRemovalIterations) {
	...
	if (isLoggingEnabled())
	  getLogger().info("Iteration #" + (i+1) + ": size after=" + result.numInstances());
      }
    }
    else {
      i     = 0;
	...
	if (result.numInstances() == old.numInstances()) {
	  ...
	  if (count >= maxNonRemovalIterations) {
	    if (isLoggingEnabled())
	      getLogger().info("Maximum number of iterations with no removals reached (" + maxNonRemovalIterations + "), exiting!");
	    break;
	  }
	}
	else {
  ...
  protected Instances performPostProcess(Instances data) {
	...
	break;
      ...
      if (isLoggingEnabled())
	getLogger().info("Using classifier #" + (i+1) + ": " + OptionUtils.getCommandLine(m_Classifiers[i]));
      result = iterate(
	result,
	m_Classifiers[i],
    ...
    if (isLoggingEnabled())
      getLogger().info("# outliers removed: " + (data.numInstances() - result.numInstances()));
