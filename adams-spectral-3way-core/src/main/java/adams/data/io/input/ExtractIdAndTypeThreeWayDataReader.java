...
/*
 * ExtractIdAndTypeThreeWayDataReader.java
 ...
 * &nbsp;&nbsp;&nbsp;For extracting the sample type from the 3-way data.
 ...
 */
public class ExtractIdAndTypeThreeWayDataReader
  ...
  public String globalInfo() {
    return "Uses the specified ID and group extractor to obtain and update "
      + "sample ID and type of the 3-way data read by the specified base reader.";
  }
  ...
  public String groupExtractionTipText() {
    return "For extracting the sample type from the 3-way data.";
  }
  ...
  protected void readData() {
    ...
    for (Enumerated<ThreeWayData> r: enumerate(m_ReadData)) {
      // ID
	...
	if (m_GroupExtraction.handles(r.value)) {
	  ...
	  if (type != null) {
	    r.value.getReport().setStringValue(ThreeWayReport.SAMPLE_TYPE, type);
	    if (isLoggingEnabled())
