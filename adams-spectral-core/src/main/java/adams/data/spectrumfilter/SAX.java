 ...
 * The data must be normalized using knir.data.filter.RowNorm beforehand.<br>
...
public class SAX
  ...
  protected double getMeanDeltaX(Spectrum data) {
    SpectrumStatistic	stats;
    
    stats = new SpectrumStatistic(data);

    return stats.getStatistic(SpectrumStatistic.MEAN_DELTA_WAVE_NUMBER);
  }
